...
function generatePasswordHistoryTable() {
  var _iterator = _createForOfIteratorHelper(historyArray),
    _step;
  try {
    for (_iterator.s(); !(_step = _iterator.n()).done;) {
      var i = _step.value;
      var tr = document.createElement('tr');
      tr.classList.add('trBoarder');
      var table = document.querySelector('.table');
      table.append(tr);
      var td = document.createElement('td');
      td.classList.add('tdPadding');
      td.innerHTML = "".concat(i.password);
      tr.prepend(td);
      var td2 = document.createElement('td');
      if (!i.siteName) {
        td2.innerHTML = 'unknown';
      } else {
        td2.innerHTML = "".concat(i.siteName);
      }
      td.after(td2);
      var td3 = document.createElement('td');
      td3.innerHTML = "".concat((0,date_fns_format__WEBPACK_IMPORTED_MODULE_0__["default"])(new Date(i.date), 'dd MMMM yyyy kk:mm:ss'));
      td2.after(td3);
    }
  } catch (err) {
    _iterator.e(err);
...
document.querySelector('.clearHistory').addEventListener('click', deleteHistory);
